...
const API_BASE_URL = (
  import.meta.env.VITE_API_BASE_URL || "https://hematbox.sugengaldi.my.id/api/v1"
).replace("/api/v1", "");
...
<template>
  <div class="bg-gradient-to-br from-slate-50 via-teal-50/30 to-slate-50 min-h-screen">
    <!-- Cart Notification -->
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div v-if="isLoading" class="text-center py-24">
        <Icon icon="line-md:loading-loop" class="mx-auto w-16 h-16 text-teal-500" />
      ...
      <div v-else-if="error" class="text-center py-24">
        <Icon icon="mdi:alert-circle-outline" class="mx-auto w-16 h-16 text-red-400" />
      ...
      <!-- Main Content -->
      <div v-else-if="business" class="grid grid-cols-1 lg:grid-cols-3 lg:gap-8">
        <!-- Sidebar -->
        <aside class="lg:col-span-1 mb-8 lg:mb-0">
          <div class="bg-white rounded-3xl shadow-xl p-6 sticky top-8 border border-teal-100">
            <!-- Business Image with Gradient Overlay -->
            <div class="relative aspect-w-16 aspect-h-9 mb-6 rounded-2xl overflow-hidden group">
              <img v-if="getImageUrl(business.image_url)" :src="getImageUrl(business.image_url)" alt="Foto Toko"
                class="object-cover w-full h-full group-hover:scale-105 transition-transform duration-500" />
              <div v-else class="bg-gradient-to-br from-teal-100 to-teal-200 flex items-center justify-center">
                <Icon icon="mdi:store-outline" class="text-6xl text-teal-400" />
              </div>
              <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent"></div>
              <div class="absolute bottom-4 left-4 right-4">
                <div class="bg-white/20 backdrop-blur-md rounded-xl px-3 py-1.5 inline-block">
            ...
            <div class="mb-4">
              ...
              <p v-if="business.description" class="text-gray-600 text-sm leading-relaxed">
            ...
            <div class="grid grid-cols-2 gap-3 mb-6">
              <div class="bg-gradient-to-br from-teal-50 to-teal-100 rounded-xl p-4 text-center">
                <Icon icon="mdi:food-variant" class="w-6 h-6 text-teal-600 mx-auto mb-1" />
              ...
              <div class="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl p-4 text-center">
                <Icon icon="mdi:star" class="w-6 h-6 text-orange-600 mx-auto mb-1" />
            ...
            <!-- Map -->
            <div v-if="mapMarker" class="h-64 rounded-2xl overflow-hidden border-2 border-teal-100 shadow-lg">
              <l-map ref="map" v-model:zoom="zoom" :center="mapCenter">
                <l-tile-layer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                  name="OpenStreetMap"></l-tile-layer>
                <l-marker :lat-lng="mapMarker.position">
                  <l-icon :icon-url="storeIconUrl" :icon-size="[36, 36]" :icon-anchor="[18, 36]" />
                  <l-popup><b>{{ mapMarker.name }}</b></l-popup>
                </l-marker>
              </l-map>
            </div>
          </div>
        ...
        <main class="lg:col-span-2">
          ...
          <div class="flex items-center justify-between mb-8">
            ...
            <div class="hidden md:block">
              <div class="bg-white rounded-2xl px-6 py-3 shadow-lg border border-teal-100">
          ...
          <div v-if="hasOffers" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div v-for="offer in offers" :key="offer.id"
              ...
              <div class="relative overflow-hidden">
                ...
                <!-- Stock Badge -->
                <div class="absolute top-4 right-4 backdrop-blur-md rounded-xl px-3 py-2 shadow-lg" :class="offer.remaining_portion <= 3
                    ? 'bg-red-500/90'
                    : 'bg-teal-500/90'
                  ">
                  <div class="flex items-center gap-1.5 text-white">
                    <Icon icon="mdi:package-variant" class="w-4 h-4" />
                    <span class="text-xs font-bold">Sisa {{ offer.remaining_portion }}</span>
                  </div>
                </div>
              ...
              <div class="p-5">
                ...
                <!-- Pickup Time -->
                <div class="bg-gradient-to-r from-orange-50 to-amber-50 rounded-xl p-3 mb-4">
                ...
                <button @click="goToOfferDetail(offer.id)" :disabled="offer.remaining_portion === 0"
                  class="w-full bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 disabled:from-gray-300 disabled:to-gray-400 text-white font-bold py-4 rounded-2xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2 group">
                  <Icon :icon="offer.remaining_portion === 0
                      ? 'mdi:cancel'
          ...
          <!-- Empty State -->
          <div v-else class="text-center py-20 bg-white rounded-3xl shadow-xl border border-teal-50">
            <div class="max-w-md mx-auto">
              <div
                ...
                <Icon icon="mdi:food-off-outline" class="w-12 h-12 text-gray-400" />
              </div>
