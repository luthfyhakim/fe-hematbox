    ...
    <div class="max-w-7xl mx-auto p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-4 mb-8 items-end">
        <div class="md:col-span-6 relative">
          <label for="search" class="block text-sm font-semibold text-gray-700 mb-1">Cari Penawaran</label>
          <Icon icon="mdi:magnify" class="w-5 h-5 text-gray-400 absolute left-4 top-10" />
          <input v-model="searchQuery" type="text" id="search" placeholder="Nasi Goreng, Roti, Kopi..."
      ...
      <div v-else>
        <div v-if="filteredOffers.length > 0"
          ...
          <div v-for="offer in filteredOffers" :key="offer.id" @click="goToOfferDetail(offer.id)"
            ...
            <div class="relative">
              <img :src="offer.image" :alt="offer.foodName" class="w-full h-48 object-cover" />
              <div
            ...
            <div class="p-4">
              <div class="flex justify-between items-center mb-3">
                ...
                <div class="text-sm font-semibold text-orange-600 bg-orange-100 px-3 py-1 rounded-full">
                  Sisa {{ offer.stock }}
                </div>
              </div>
              <div class="text-xs text-gray-500 flex justify-between items-center mb-4">
                <div class="flex items-center gap-1.5">
                  <Icon icon="mdi:map-marker-outline" /><span>{{ offer.distance }} km</span>
                </div>
                <div class="flex items-center gap-1.5">
        ...
        <div v-else class="text-center py-24">
          <Icon icon="mdi:food-off-outline" class="mx-auto w-16 h-16 text-gray-400" />
          <h3 class="mt-4 text-xl font-semibold text-gray-700">
        ...
        <section class="mt-16">
          ...
          <div class="h-[500px] rounded-2xl border border-gray-200 overflow-hidden shadow-xl">
            <l-map ref="map" v-model:zoom="zoom" :center="mapCenter" :use-global-leaflet="false">
              <l-tile-layer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" layer-type="base"
                name="OpenStreetMap"
                attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'></l-tile-layer>

              <l-marker v-for="marker in mapMarkers" :key="marker.id" :lat-lng="marker.position">
                <l-icon :icon-size="[0, 0]" :icon-anchor="[20, 40]" class-name="custom-marker-icon">
                ...
                <l-popup :options="{ maxWidth: 320, className: 'custom-popup' }">
                  <div
                    ...
                    <div class="p-5 pt-6">
                      ...
                      <!-- Address with icon -->
                      <div class="flex items-start gap-2 mb-3">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-gray-400 flex-shrink-0 mt-0.5"
                      ...
                      <div v-if="marker.phone && marker.phone !== 'No Telp'"
                      ...
                      <button @click="goToBusinessProfile(marker.id)"
